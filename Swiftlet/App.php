<?php
class Swiftlet_App implements Swiftlet_Interfaces_App {
    ...
    public function run() {
        ...
        $this->view = new Swiftlet_View($this, strtolower($controllerName));

        // Instantiate the controller
        $controllerName = 'Swiftlet_Controllers_' . basename($controllerName);
        ...
        if ($handle = opendir('Swiftlet/Plugins')) {
            while (($file = readdir($handle)) !== false) {
                if (is_file('Swiftlet/Plugins/' . $file) && preg_match('/^(.+)\.php$/', $file, $match)) {
                    $pluginName = 'Swiftlet_Plugins_' . $match[1];
    ...
    public function getModel($modelName) {
        $modelName = 'Swiftlet_Models_' . ucfirst($modelName);
